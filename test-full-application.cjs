...
async function testAPIEndpoints() {
  ...
  const endpoints = [
    ...
    { path: '/api/customers', validation: (res) => ({ 
      passed: Array.isArray(res.data?.customers) && res.data.customers.length > 0, 
      message: `Found ${res.data?.customers?.length || 0} customers` 
    })},
    ...
    { path: '/api/supplier-quotes', validation: (res) => ({ 
      passed: Array.isArray(res.data?.data) && res.data.data.length > 0, 
      message: `Found ${res.data?.data?.length || 0} supplier quotes` 
    })},
...
async function testDatabaseOperations() {
  ...
  try {
    ...
    logTest('Create Customer', passed, message, {
