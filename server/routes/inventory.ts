...
export function registerInventoryRoutes(app: Express) {
  ...
  app.post("/api/stock-movements", async (req, res) => {
    try {
      const movementData = {
        ...
        movementType: req.body.movementType || "Transfer",
        referenceType: req.body.referenceType || "Transfer",
  ...
  app.put("/api/stock-movements/:id", async (req, res) => {
    try {
      const quantityMoved = Number(req.body.quantityMoved ?? req.body.quantity);
      const quantityBefore = Number(req.body.quantityBefore ?? 0);
      const quantityAfter = Number(req.body.quantityAfter ?? quantityBefore + (isNaN(quantityMoved) ? 0 : quantityMoved));
      const movementData = {
        itemId: req.body.itemId,
        variantId: req.body.variantId,
        movementType: req.body.movementType || "Transfer",
        ...
        referenceId: req.body.referenceId,
        transferNumber: req.body.transferNumber,
        quantityMoved: isNaN(quantityMoved) ? undefined : quantityMoved,
        quantityBefore: isNaN(quantityBefore) ? undefined : quantityBefore,
        quantityAfter: isNaN(quantityAfter) ? undefined : quantityAfter,
        fromLocation: req.body.fromLocation,
        toLocation: req.body.toLocation,
        storageLocation: req.body.storageLocation,
        transferDate: req.body.transferDate,
      ...
      const movement = await storage.updateStockMovement(req.params.id, movementData);
      if (!movement) {
        return res.status(404).json({ message: "Stock movement not found" });
      }
