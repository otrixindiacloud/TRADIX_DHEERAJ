...
export function registerSalesOrderRoutes(app: Express) {
  ...
  // Recalculate financial totals for all sales orders
  app.post("/api/sales-orders/recalculate-totals", async (req, res) => {
    try {
      await storage.recalculateAllFinancialTotals();
      res.json({ message: "Financial totals recalculated successfully for all sales orders" });
    } catch (error) {
      console.error("Error recalculating financial totals:", error);
      res.status(500).json({ message: "Failed to recalculate financial totals", error: error instanceof Error ? error.message : 'Unknown error' });
    }
  });

  // Recalculate financial totals for a specific sales order
  app.post("/api/sales-orders/:id/recalculate-totals", async (req, res) => {
    try {
      await storage.updateFinancialTotals(req.params.id);
      res.json({ message: "Financial totals recalculated successfully for sales order" });
    } catch (error) {
      console.error("Error recalculating financial totals for sales order:", error);
      res.status(500).json({ message: "Failed to recalculate financial totals", error: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
}
