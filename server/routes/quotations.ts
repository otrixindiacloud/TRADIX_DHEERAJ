...
export function registerQuotationRoutes(app: Express) {
  ...
  app.delete("/api/quotations/:id", async (req, res) => {
    try {
      const { id } = req.params;
      console.log(`[DELETE /api/quotations/${id}] Starting deletion process`);
      
      await storage.deleteQuotation(id);
      console.log(`[DELETE /api/quotations/${id}] Deletion successful`);
      res.status(204).send();
    } catch (error) {
      console.error(`[DELETE /api/quotations/${req.params.id}] Error:`, error);
      
      if (error instanceof Error) {
        if (error.message === 'Quotation not found') {
          return res.status(404).json({ message: "Quotation not found" });
        }
        if (error.message.includes('referenced by') || error.message.includes('Cannot delete quotation')) {
        ...
        if ('code' in error && error.code === '23503') {
          return res.status(400).json({ 
            message: "Cannot delete quotation. It is referenced by other records. Please delete the related records first." 
          });
        }
