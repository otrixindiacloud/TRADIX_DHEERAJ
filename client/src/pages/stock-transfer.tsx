...
export default function StockTransferPage() {
  ...
  React.useEffect(() => {
    if (showEditDialog && selectedTransfer) {
      const normalizedStatus = TRANSFER_STATUSES.includes(selectedTransfer.status)
        ? selectedTransfer.status
        : "Draft";
      const resolvedQuantity = Number(selectedTransfer.quantityMoved ?? selectedTransfer.quantity ?? 0);
      form.reset({
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, transfer: any) => {
        const canEdit = (transfer.recordType ?? "Transfer") !== "Issue" && Boolean(transfer.id);
        return (
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
            ...
            <Button
              ...
              onClick={() => {
                if (!canEdit) {
                  toast({
                    title: "Not Supported",
                    description: "Only stock transfers can be edited from this view.",
                    variant: "destructive",
                  });
                  return;
                }
                  ...
                  <DialogContent className="max-w-6xl max-h-[85vh] overflow-y-auto">
                    ...
                    <Form {...form}>
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        <div className="grid grid-cols-3 gap-4 bg-slate-50 rounded-xl p-4 border border-slate-200/60">
                          <FormField
            ...
            <Form {...form}>
              <form
                onSubmit={async (e) => {
                  ...
                  if (!selectedTransfer || (selectedTransfer.recordType ?? "Transfer") === "Issue") {
                    toast({
                      title: "Error",
                      description: "Only stock transfers can be edited from this view.",
                      variant: "destructive",
                  ...
                  const selectedItem = items.find((item: any) => item.id === formData.itemId);
                  const baseUnitCost = selectedItem?.unitCost ?? selectedTransfer.unitCost ?? 0;
                  const numericUnitCost = typeof baseUnitCost === "number" ? baseUnitCost : parseFloat(baseUnitCost || "0");
                  const resolvedQuantityBefore = selectedTransfer.quantityBefore != null
                    ? Number(selectedTransfer.quantityBefore)
                    : selectedTransfer.quantityAfter != null && selectedTransfer.quantityMoved != null
                      ? Number(selectedTransfer.quantityAfter) - Number(selectedTransfer.quantityMoved)
                      : 0;
                  const quantityBefore = Number.isFinite(resolvedQuantityBefore) ? resolvedQuantityBefore : 0;
                  const rawQuantity = Number(formData.quantity ?? 0);
                  const quantity = Number.isFinite(rawQuantity) ? rawQuantity : 0;
                  const safeUnitCost = Number.isFinite(numericUnitCost) ? numericUnitCost : 0;
                  const totalValueNumber = safeUnitCost * quantity;
                  const formattedUnitCost = safeUnitCost.toFixed(2);
                  const formattedTotalValue = totalValueNumber.toFixed(2);
                  const referenceId = selectedTransfer.referenceId || selectedTransfer.transferNumber || formData.transferNumber;

                  const updatePayload = {
                    itemId: formData.itemId,
                    variantId: selectedTransfer.variantId || selectedItem?.variantId,
                    movementType: "Transfer",
                    referenceType: "Transfer",
                    referenceId,
                    ...
                    storageLocation: formData.fromLocation,
                    transferDate: formData.transferDate,
                    requestedBy: formData.requestedBy,
                    createdBy: selectedTransfer.createdBy || formData.requestedBy,
                        ...
                        <FormControl>
                          <Input
                            ...
                            onChange={(event) => {
                              ...
                              field.onChange(nextValue === "" ? 0 : parseInt(nextValue, 10) || 0);
                            }}
                          />
                        </FormControl>
                        <FormMessage />
