...
export default function RequisitionsPage() {
  ...
  const updateRequisition = useMutation({
    mutationFn: async (data: Partial<Requisition> & { id: string }) => {
      // For updates, we don't allow changing the customer (requestedBy/requestedById)
  ...
  const handleEdit = (requisition: Requisition) => {
    // Format the required date for the date input (YYYY-MM-DD format)
    const formattedRequiredDate = requisition.requiredDate 
      ? new Date(requisition.requiredDate).toISOString().split('T')[0] 
      : '';
            ...
            <div className="space-y-4">
                  ...
                  <div className="border border-gray-200 rounded-md px-3 py-2 bg-gray-50">
                    <div className="flex flex-col">
                      <span className="font-medium text-gray-900">
                        {(() => {
                          // Find customer if requestedById exists
                          if (editingRequisition.requestedById && customersData && Array.isArray(customersData)) {
                            const customer = customersData.find((c: Customer) => c.id === editingRequisition.requestedById);
                            return customer?.name || editingRequisition.requestedBy || "Unknown Customer";
                          }
                          return editingRequisition.requestedBy || "Unknown Customer";
                        })()}
                      </span>
                      {(() => {
                        // Show customer type and classification if available
                        if (editingRequisition.requestedById && customersData && Array.isArray(customersData)) {
                          const customer = customersData.find((c: Customer) => c.id === editingRequisition.requestedById);
                          if (customer) {
                            return (
                              <span className="text-sm text-gray-500">
                                {customer.customerType} - {customer.classification}
                              </span>
                            );
                          }
                        }
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button 
                  ...
                  disabled={updateRequisition.isPending || !editingRequisition.department || !editingRequisition.justification}
                >
