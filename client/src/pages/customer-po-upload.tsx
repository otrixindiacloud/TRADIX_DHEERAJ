...
export default function PoUpload() {
  ...
  // Items for viewing dialog
  const { data: viewingItems = [] } = useQuery({
    queryKey: ["/api/quotations", viewingQuotation?.id, "items"],
    enabled: !!viewingQuotation?.id,
    queryFn: async () => {
      const res = await fetch(`/api/quotations/${viewingQuotation?.id}/items`);
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          {viewingQuotation && (
            <div className="space-y-6">
              ...
              {/* Items */}
              {Array.isArray(viewingItems) && viewingItems.length > 0 && (
                <div className="border-t pt-4">
                  <h4 className="font-medium text-gray-900 mb-3">Items</h4>
                  <div className="space-y-3">
                    {viewingItems.map((it: any, idx: number) => (
                      <div key={it.id || idx} className="border rounded-lg p-4">
                        {it.description && (
                          <div className="mb-2">
                        ...
                        <div className="grid grid-cols-4 gap-4">
                          ...
                          <div>
                            ...
                            <p className="font-medium">{formatCurrency(Number(it.unitPrice) || 0)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-600">Line Total</p>
                            <p className="font-medium">{formatCurrency(Number(it.lineTotal) || 0)}</p>
                          </div>
                          {it.unitOfMeasure && (
                            <div>
                              <p className="text-xs text-gray-600">UOM</p>
                              <p className="font-medium">{it.unitOfMeasure}</p>
                            </div>
                          )}
                        </div>
                      </div>
