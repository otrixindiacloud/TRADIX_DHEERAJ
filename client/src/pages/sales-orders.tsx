...
export default function SalesOrders() {
  ...
  const { data: selectedOrderItems = [] } = useQuery<SalesOrderItemWithSupplier[]>({
    ...
    queryFn: async () => {
      if (!selectedOrder?.id) return [] as SalesOrderItemWithSupplier[];
      const res = await fetch(`/api/sales-orders/${selectedOrder.id}/items`);
      if (!res.ok) throw new Error("Failed to fetch sales order items");
      return res.json();
    },
  });

  // Also fetch quotation items for the linked quotation to enrich item descriptions if needed
  const { data: selectedQuotationItems = [] } = useQuery<any[]>({
    queryKey: ["/api/quotations", selectedOrder?.quotationId, "items"],
    enabled: !!selectedOrder?.quotationId,
    queryFn: async () => {
      const res = await fetch(`/api/quotations/${selectedOrder?.quotationId}/items`);
      if (!res.ok) throw new Error("Failed to fetch quotation items");
      return res.json();
    },
  });

  // Merge: if a sales order item has a generic or empty description, fall back to quotation item description/notes by index
  const enrichedSelectedOrderItems = (selectedOrderItems || []).map((it, idx) => {
    const qi = (selectedQuotationItems || [])[idx];
    const isGeneric = !((it as any).description) || String((it as any).description).toLowerCase().includes("generic item");
    const merged: any = { ...it };
    if (qi && (isGeneric || !(it as any).description)) {
      merged.description = qi.description || merged.description;
    }
    if (qi && !merged.specialInstructions && qi.notes) {
      merged.specialInstructions = qi.notes;
    }
    return merged;
  });
  ...
  const columns: Column<SalesOrderWithRelations>[] = [
    ...
    },
    {
      ...
      render: (_: any, order: SalesOrderWithRelations) => {
        ...
        const quoteNumber = quotation?.quoteNumber || (order as any).quotationId;
        if (!quoteNumber) {
        ...
        return (
          <div className="flex flex-col">
            ...
            {quotation?.revision && quotation.revision > 1 && (
          ...
          {selectedOrder && (
            <Tabs defaultValue="details" className="w-full">
              <TabsList className="grid w-full grid-cols-3">
                ...
                <TabsTrigger value="items">Items ({enrichedSelectedOrderItems.length})</TabsTrigger>
              ...
              <TabsContent value="items" className="space-y-4">
                <div className="space-y-3">
                  {enrichedSelectedOrderItems.map((item, index) => (
                    <div key={item.id} className="border rounded-lg p-4">
                      {/* Primary item description from catalog/original item */}
                      {(item as any).description || (item as any).itemDescription ? (
                        <div className="mb-3">
                          <p className="text-xs text-gray-600">Item Description</p>
                          <p className="text-sm">{(item as any).description || (item as any).itemDescription}</p>
                        </div>
                      ) : null}
                      ...
                      {/* Line-level description/notes from the sales order line */}
                      ...
                      {((item as any).category || (item as any).unitOfMeasure) && (
                        <div className="mt-2 grid grid-cols-2 gap-4">
                          {(item as any).category && (
                            <div>
                              <p className="text-xs text-gray-600">Category</p>
                              <p className="text-sm">{(item as any).category}</p>
                            </div>
                          )}
                          {(item as any).unitOfMeasure && (
                            <div>
                              <p className="text-xs text-gray-600">UOM</p>
                              <p className="text-sm">{(item as any).unitOfMeasure}</p>
                            </div>
                          )}
                        </div>
                      )}
                  ...
                  {selectedOrderItems.length === 0 && (
                    <p className="text-center text-gray-500 py-8">No items have been added to this sales order yet.</p>
                  )}
                </div>
              </TabsContent>
