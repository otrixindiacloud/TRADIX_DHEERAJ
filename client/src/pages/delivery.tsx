...
export default function Delivery() {
  ...
  // Fetch delivery items for the view dialog and show original item details
  const { data: viewingDeliveryItems = [] } = useQuery<any[]>({
    queryKey: ["/api/deliveries", selectedDeliveryForDetails?.id, "items"],
    enabled: !!selectedDeliveryForDetails?.id,
    queryFn: async () => {
      const res = await fetch(`/api/deliveries/${selectedDeliveryForDetails?.id}/items`);
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedDeliveryForDetails && (
            <div className="space-y-6">
              ...
              {/* Delivery Items */}
              {Array.isArray(viewingDeliveryItems) && viewingDeliveryItems.length > 0 && (
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    <Package className="h-4 w-4 text-gray-600" />
                    Delivery Items ({viewingDeliveryItems.length})
                  </h4>
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-left font-medium text-gray-900">Supplier Code</th>
                            <th className="px-3 py-2 text-left font-medium text-gray-900">Barcode</th>
                        ...
                        <tbody>
                          {viewingDeliveryItems.map((it: any, idx: number) => {
                            const qty = it.deliveredQuantity ?? it.pickedQuantity ?? it.orderedQuantity ?? it.quantity;
                            return (
                              <tr key={it.id || idx} className="border-b border-gray-100">
                                <td className="px-3 py-2 text-gray-900">{it.description || it.itemDescription || 'Item'}</td>
                                <td className="px-3 py-2 text-gray-700 font-mono text-xs">{it.supplierCode || it.item?.supplierCode || ''}</td>
                                <td className="px-3 py-2 text-gray-700 font-mono text-xs">{it.barcode || it.item?.barcode || ''}</td>
                                <td className="px-3 py-2 text-center text-gray-700">{qty}</td>
                                <td className="px-3 py-2 text-right text-gray-700">{formatCurrency(Number(it.unitPrice || 0))}</td>
                                <td className="px-3 py-2 text-right font-medium text-gray-900">{formatCurrency(Number(it.totalPrice || 0))}</td>
                              </tr>
                            );
                          })}
                        </tbody>
