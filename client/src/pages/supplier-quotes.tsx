...
interface SupplierQuote {
  ...
  rfqNumber?: string;
  status: "Draft" | "Sent" | "Accepted" | "Approved" | "Rejected" | "Expired";
...
export default function SupplierQuotesPage() {
  ...
  const [editQuote, setEditQuote] = useState({
    ...
    deliveryTerms: "",
    notes: "",
    status: "Draft" as "Draft" | "Sent" | "Accepted" | "Approved" | "Rejected" | "Expired",
    currency: "BHD" as "BHD" | "AED" | "USD" | "EUR" | "GBP",
  });
  ...
  const updateSupplierQuote = useMutation({
    mutationFn: async (data: { id: string; updates: typeof editQuote }) => {
      console.log("Updating supplier quote with data:", data);
      ...
      // Process the updates to only include valid fields and exclude nulls
      const processedUpdates: any = {};
      
      // Only include fields that have values and are allowed to be updated
      if (data.updates.supplierId) processedUpdates.supplierId = data.updates.supplierId;
      if (data.updates.status) processedUpdates.status = data.updates.status;
      if (data.updates.priority) processedUpdates.priority = data.updates.priority;
      if (data.updates.currency) processedUpdates.currency = data.updates.currency;
      if (data.updates.totalAmount) processedUpdates.totalAmount = data.updates.totalAmount;
      if (data.updates.paymentTerms) processedUpdates.paymentTerms = data.updates.paymentTerms;
      if (data.updates.deliveryTerms) processedUpdates.deliveryTerms = data.updates.deliveryTerms;
      if (data.updates.notes) processedUpdates.notes = data.updates.notes;
      
      // Convert validUntil string to ISO date string for backend
      if (data.updates.validUntil) {
        const date = new Date(data.updates.validUntil);
      ...
      console.log("Processed updates being sent:", processedUpdates);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {/* Request Quote button removed per product request */}
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: "Draft" | "Sent" | "Accepted" | "Approved" | "Rejected" | "Expired") => 
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  type="number"
                  step="0.01"
                  value={editQuote.totalAmount}
                  onChange={(e) => setEditQuote({ ...editQuote, totalAmount: e.target.value })}
                  placeholder="0.00"
                />
              </div>
            </div>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      {selectedQuotes.map(quoteId => {
                        ...
                        return (
                          <td key={quoteId} className="border border-gray-200 p-3">
                            <StatusPill status={quote?.status || 'unknown'} />
                          </td>
                        );
                      })}
                    </tr>
