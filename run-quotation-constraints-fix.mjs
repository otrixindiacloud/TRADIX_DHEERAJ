...
async function runQuotationConstraintsFix() {
  try {
    console.log("Running migration: Fix purchase order foreign key constraints for quotation deletion");
    ...
    const migrationPath = path.join(__dirname, "one-time-scripts", "fix-quotation-purchase-order-constraints.sql");
    ...
    console.log('✅ Purchase order foreign key constraints fix completed successfully!');
    console.log('Fixed foreign key constraints:');
    console.log('  - purchase_orders.quotation_id now has SET NULL on delete');
    console.log('');
